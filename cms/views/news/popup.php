...
?>
    ...
    <div class="col-12" style="padding: 0px 0px 10px 0px !important;">
            <?php echo Html::input('number', 'id',  isset($_REQUEST['id']) ? ArrayHelper::getValue($_REQUEST, 'id', null) : null, ['class' => 'form-control', 'placeholder'=>Yii::t('cms', 'Nhập ID bài viết')]); ?>
            <?php echo Html::input('text', 'title',  isset($_REQUEST['title']) ? ArrayHelper::getValue($_REQUEST, 'title', null) : null, ['class' => 'form-control', 'placeholder'=>Yii::t('cms', 'Nhập title')]); ?>
			<div class="form-group">
				<select class="form-control" name="news_category_id" id="news_category_id">
					<option value=''>Chọn danh mục</option>
					<?php foreach($selectBox as $v): ?>
					<option value="<?= $v['id'] ?>" <?php if($_REQUEST['news_category_id'] == $v['id']) echo 'selected'; ?>><?= $v['title'] ?></option>
					<?php endforeach; ?>
				</select>
			</div>
...
<script>
    ...
    $(document).on("click", "#btn-search-popup", function(e){
        e.preventDefault();
        e.stopPropagation();
        url = '/news/popup-pagination.html?title=' + $('input[name=title]').val() + '&id=' + $('input[name=id]').val() + '&news_category_id=' + $('#news_category_id').val();
        $.get(url)
            .done(function (data) {
                $("#grid-ajax-news").html(data);
            })
