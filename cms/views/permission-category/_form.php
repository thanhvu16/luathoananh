...
$category = NewsCategory::find()->orderBy('is_hot DESC, order ASC')->asArray()->all();
$sys = new CategoryTree($category);
$category = $sys->builArray(0);
$selectBox = $sys->selectboxArray($category, 'title');
?>
<div class="news-form">
    ...
    <?php echo $form->field($model, 'pseudonym')->textInput(['maxlength' => 255]); ?>
    ...
    <?= $form->field($model, 'news_category_id')->label('Chọn danh mục')->dropDownList(ArrayHelper::map($selectBox, 'id', 'name'), ['prompt' => Yii::t('cms', 'Chọn danh mục')]) ?>
...
<!--    --><?php //echo $form->field($model, 'match_id')->textInput(['maxlength' => 255, 'type' => 'number'])->label('ID trận đấu'); ?>
    ...
    <?php echo $form->field($model,'timeStartMatchFormat')->widget(\kartik\datetime\DateTimePicker::className(),[
        'options' => ['style' => 'width: 200px;'],
        'pluginOptions' => ['autoclose' => true,'format' => 'yyyy-mm-dd hh:ii:ss'],
    ])?>
	
	
    <?php echo $form->field($model, 'is_hot')->checkbox(['label' => Yii::t('cms', 'Bài viết HOT')])->label(false) ?>
    <div class="form-group">
