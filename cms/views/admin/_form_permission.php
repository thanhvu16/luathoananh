<?php
    ...
    $this->params['title'] = Html::encode(Yii::t('cms', 'admin_permission'));
?>
<?php $form = ActiveForm::begin([
    'method' => 'post',
    'action' => ['admin/permission-add'],
]); ?>
    ...
    <?php echo GridView::widget([
        ...
        'columns' => [
            ...
            [
                'attribute' => Yii::t('cms', 'permission'),
                'value' => function ($data) {
                    return AdminAction::checkboxListCustomized(
                        $data['controller'],
                        isset($data['permission']) ? $data['permission'] : [],
                        isset($data['attributes']) ? $data['attributes'] : [] ,
                        ['item'=>function ($index, $label, $name, $checked, $value, $params) {
                        return '
                            <div id="admin-action-delete-'.$params['id'].'">'.
                                Html::checkbox($name, $checked, [
                                    'value' => $value,
                                    'label' => '<span class="admin-action-label">'.$value.'</span>
                                                <a class="admin-action-operations" href="javascript:void(0)" data-toggle="tooltip" data-placement="top" title="'.Yii::t('cms', 'delete_action').'"><i class="glyphicon glyphicon-remove" onclick="deleteAction('.$label->id.')"></i></a>
                                                <a class="admin-action-operations" href="javascript:void(0)" data-toggle="tooltip" data-placement="top" title="'.Yii::t('cms', 'edit_desc').'"><i class="glyphicon glyphicon-pencil" onclick="updateDescAction('.$label->id.', \''.$label->desc.'\')"></i></a>
                                                <span class="admin-action-desc" id="admin-action-desc-'.$params['id'].'">'.$label->desc.'</span>',
                                    'class' => 'checkbox_action_'.substr($name, 0, -2)
                                ])
                        .'</div>';
                    }, 'class' => 'checkbox_action'], $data);
